...
<div class="bg-white rounded-lg shadow p-8">
    ...
    <form method="POST" action="{{ route('quiz.verify.submit', $assignment->public_token) }}" class="space-y-4">
        @csrf
    ...
    <form method="POST" action="{{ route('quiz.resend', $assignment->public_token) }}" class="mt-4">
        @csrf
        <input type="hidden" name="session_id" value="{{ session('session_id') ?? old('session_id') }}">
